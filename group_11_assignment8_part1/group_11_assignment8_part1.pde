...
void drawBaseGraph() {
  background(255);
  strokeWeight(4);
  line(100, height-98, width-60, height-98);
...
void drawBarGraph() {
  ...
  for (int i = 1; i<numbers.length; i++) {
    if (i <= 16) {
      ...
      if (i%4==1) {
        stroke(255);
        for (int j=0 ; j<=790+h-height; j+=28){
          line(((i-15)*2*w)+0.22*w, height-(h+100)+j, ((i-15)*2*w)+1.05*w, height-h-70+j);        
        }
       }
       else {
        stroke(0);
        for (int j=0 ; j<=783+h-height; j+=28){
          line(((i-15)*2*w)+0.22*w, height-(h+100)+j, ((i-15)*2*w)+1.03*w, height-h-70+j);
        }
      }
    }
  }
...
void BarGraphLabel() {
  ...
  fill(0);
  text("FEMALE", 990, 385);
  line(960,370,980,380);
  line(960,380,980,390);
}
